...
class MainRepository @Inject constructor(ctxObj : Application) : BaseRepository(ctxObj) {
    suspend fun fetchAllContentFromDB(responseHandler : (ApiResource<List<Content>>) -> Unit){
        val database = AppDatabase.getDatabase(ctxObj)
        responseHandler.invoke(ApiResource.Success(database.contentDao().getAllContent()))
    }
